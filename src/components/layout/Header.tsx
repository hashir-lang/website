...
const NAV: NavItem[] = [
  {
    ...
    to: "/programmes",
    children: [
      { label: "Programmes & Diplomas", to: "/programmes", desc: "Browse the full catalogue of online programmes." },
      { label: "All Courses",            to: "/courses",    desc: "Filter by subject, level, university, and intake." },
      { label: "Scholarships",           to: "/scholarship", desc: "Tuition support for motivated students." },
    ],
  },
  {
    label: "Institution",
  ...
  return (
    ...
    >
      ...
      <div className="container-wide flex h-16 md:h-20 items-center justify-between gap-6">
        ...
        >
          <span className="font-serif text-[28px] md:text-[32px] font-medium tracking-[-0.025em] text-ink leading-none">
            Ue<span className="italic-serif text-plum">Campus</span>
          </span>
          <span className="hidden md:inline-block h-1.5 w-1.5 rounded-full bg-ember translate-y-[-2px] transition-smooth group-hover:scale-150" />
        </Link>
          ...
          {NAV.map((item) => {
            if (item.children) {
              ...
              return (
                ...
                >
                  <button
                    className={`text-[13px] font-medium tracking-tight transition-snap ${
